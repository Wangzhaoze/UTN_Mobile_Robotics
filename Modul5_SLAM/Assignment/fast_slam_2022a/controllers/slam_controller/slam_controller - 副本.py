...
def eval_sensor_model(measurements, particles):
    ...
    for particle in particles:
        ...
        for i, id in enumerate(ids):
            z_t_i = np.array([particle['x']+x_dists[i], particle['y']+y_dists[i]])

            if particle['landmarks'][id]['observed']:                
                z_hat = np.dot(C_t, particle['landmarks'][id]['mu'].reshape((2, 1))) #2, 1

                Q = np.dot(C_t, particle['landmarks'][id]['sigma']).dot(C_t.T) + R_t
                K = np.dot(particle['landmarks'][id]['sigma'], C_t).dot(np.linalg.inv(Q))
                delta = z_t_i.reshape((2, 1)) - z_hat
                mu = np.dot(K, delta)
                particle['landmarks'][id]['mu'] += mu
                particle['landmarks'][id]['sigma'] = ((C_t-np.dot(K, C_t)).dot(particle['landmarks'][id]['sigma'])).T
                #particle['weight'] = particle['weight']* sample_normal(z_hat, Q)
                
            else:
                particle['landmarks'][id]['mu'] = z_t_i
                particle['landmarks'][id]['sigma'] = R_t
...
def resample_particles(particles):
    ...
    for i in range(len(new_weights)):
        particles[i]['weights'] = new_weights[i]
...
def main():
    ...
    while robot.step(timestep) != -1:
        ...
        particles = eval_sensor_model(measurements, particles)
