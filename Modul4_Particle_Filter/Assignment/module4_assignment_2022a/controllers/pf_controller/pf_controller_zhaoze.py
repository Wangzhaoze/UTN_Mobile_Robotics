...
##########################################################################################
# This is new version of get_odometry, but still some small bugs that I couldn't solve
# please make ‘wheel_odometry‘ is True if we want to use it
# else we get odometry still from robot real position 
##########################################################################################
wheel_odometry = False

def get_odometry_from_wheel(t0_left, t0_right, t1_left, t1_right, last_pose):

    wheel_dim = 0.195
    l = 0.34

    delta_left_rad = t1_left - t0_left
    delta_right_rad = t1_right - t0_right


    dis_left = delta_left_rad * wheel_dim * 0.5
    dis_right = delta_right_rad * wheel_dim * 0.5


    if abs(dis_right-dis_left) > 10:
        curr_pose_from_wheel = last_pose

    elif abs(dis_right-dis_left) < 1e-05:
        delta_theta = 0.0
        delta_x = dis_left * np.cos(last_pose[2])
        delta_y = dis_left * np.sin(last_pose[2])
        curr_pose_from_wheel = [last_pose[0] + delta_x, last_pose[1] + delta_y, last_pose[2] + delta_theta]

    else:
        R = 0.5 * l * ((dis_left+dis_right)/(dis_right-dis_left))

        delta_theta = (dis_right-dis_left) / l
        delta_x = R * np.cos(delta_theta)-R
        delta_y = R * np.sin(delta_theta)
        curr_pose_from_wheel = [last_pose[0] + delta_x, last_pose[1] + delta_y, last_pose[2] + delta_theta]
        
    curr_pose_from_wheel[2] = normalize_angle(curr_pose_from_wheel[2])

    # reuse the given 'get_odometry' after gettinf current pose from wheel encoder
    odometry = get_odometry(last_pose, curr_pose_from_wheel)
...
def distance_to_closest_border(x, y, theta, map_limits):
    ...
    P = np.zeros((4, 2))
    P[0] = [x_min, -(A*x_min+C)/B]
    P[1] = [x_max, -(A*x_max+C)/B]
    P[2] = [-(B*y_min+C)/A, y_min]
    P[3] = [-(B*y_max+C)/A, y_max]

    del_idx = [0, 1, 2, 3]

    # find points out of map limit
    for i, px in enumerate(P[:, 0]):
            if px < x_min or px > x_max:
                del_idx.remove(i)

    for j, py in enumerate(P[:, 1]):
    ...
    if len(del_idx) == 2:
        ...
        min_dis = np.sqrt(np.square(p[0]-x)+np.square(p[1]-y))
...
def resample_particles(particles, weights):
    ...
    for ip in range(NP):
        while re_sample_id[ip] > w_cum[ind]:
            ind += 1
        re_particles.append(particles[ind])
        weights[ip] = w_cum[ip] 

    return re_particles, weights
...
def main():
    ...
    # initialize state of wheel encoder
    if wheel_odometry:
        past_right = 0.0
        past_left = 0.0       
    ...
    while robot.step(timestep) != -1:
        ...
        if wheel_odometry:
            ...
            # get odometry from wheel encoder and difference of wheel rad-angle
            odometry = get_odometry_from_wheel(past_left, past_right, leftSensor.getValue(), rightSensor.getValue(), last_pose)

            # update state of wheel encoder
            past_left = leftSensor.getValue()
            past_right = rightSensor.getValue()
